"""
Base oil trading strategy: long-only SMA crossover backtest using yfinance data.
...
def _parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Base SMA crossover backtest on oil (yfinance).")
    p.add_argument(
        "--ticker",
        type=str,
...
def main() -> None:
    ...
    from plot_results import plot_equity_and_drawdown, plot_price_and_position
    ...
    sig = add_sma_signals(close, args.fast, args.slow)
    ...
    print()
    print("=== Base strategy: long-only SMA crossover ===")
    print(f"Ticker: {args.ticker}  |  Fast/Slow: {args.fast}/{args.slow}  |  Initial cash: ${INITIAL_CASH:,.0f}")
    print(f"Commission (per side, on notional): {COMMISSION_RATE:.4%}")
    print()
    print(f"{'Metric':<22} {'SMA strategy':>16} {'Buy & hold':>16}")
    print("-" * 56)
    ...
    title = f"{args.ticker} — SMA {args.fast}/{args.slow} vs buy & hold"
    eq_path = out_dir / "equity_drawdown.png"
    px_path = out_dir / "price_smas_position.png"

    # Shading uses one-day-lagged signal (actual held position), matching backtest mechanics.
    plot_df = sig.copy()
    ...
    plot_price_and_position(
        plot_df,
        title=f"{args.ticker} — price and SMAs (green = long, executed)",
        save_path=px_path,
        show=show_plots,
    )
