"""Charts for mean-reversion backtest vs buy-and-hold."""
...
def plot_price_rsi_position(
    ...
    """
    df must include: close, rsi, position (0/1) — position should reflect held exposure
    (e.g. signal lagged one day).
    """
    fig, axes = plt.subplots(2, 1, figsize=(11, 7), sharex=True, gridspec_kw={"height_ratios": [2, 1]})
    ...
    ax_r = axes[1]
    ax_r.plot(df.index, df["rsi"], label="RSI", color="#9467bd", linewidth=1.0)
    ax_r.axhline(oversold, color="#2ca02c", linestyle="--", linewidth=0.9, label=f"Oversold ({oversold:g})")
    ax_r.axhline(overbought, color="#d62728", linestyle="--", linewidth=0.9, label=f"Overbought ({overbought:g})")
    ax_r.set_ylim(0, 100)
    ax_r.set_ylabel("RSI")
