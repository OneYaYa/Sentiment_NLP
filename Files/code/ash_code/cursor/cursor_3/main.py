"""
Oil strategy: walk-forward logistic regression on lagged returns (predict next-day up move).
...
def _parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Walk-forward logistic regression backtest on oil (yfinance).")
    p.add_argument(
        "--ticker",
        type=str,
...
def main() -> None:
    ...
    from strategy import walk_forward_logistic_positions
    ...
    sig = walk_forward_logistic_positions(
        close,
    ...
    print()
    print("=== Base strategy: walk-forward logistic regression (directional) ===")
    print(
        f"Ticker: {args.ticker}  |  train_window={args.train_window}  n_lags={args.n_lags}  "
        f"P(up)>{args.prob_threshold:g}  |  Initial cash: ${INITIAL_CASH:,.0f}"
    )
    print(f"Commission (per side, on notional): {COMMISSION_RATE:.4%}")
    print()
    print(f"{'Metric':<22} {'Logistic':>16} {'Buy & hold':>16}")
    print("-" * 56)
    ...
    title = f"{args.ticker} — logistic (lags={args.n_lags}, W={args.train_window}) vs buy & hold"
