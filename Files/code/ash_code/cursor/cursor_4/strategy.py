"""
Walk-forward random forest: predict P(next-day return > 0) from lagged returns.
...
def walk_forward_random_forest_positions(
    close: pd.Series,
    train_window: int,
    n_lags: int,
    ...
    """
    For each k >= train_window, fit RandomForestClassifier on rows [k-train_window, k-1],
    ...
    """
    if train_window < 20:
        raise ValueError("train_window should be at least ~20 for stable fits.")
    if n_lags < 1:
