"""
Meta-labeling on SMA: primary SMA long-only, gated by walk-forward logistic P(next day up).
...
def _parse_args() -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(description="Meta-labeling SMA + logistic gate on oil (yfinance).")
...
def main() -> None:
    ...
    from plot_results import plot_equity_and_drawdown, plot_price_meta_prob
    from strategy import walk_forward_meta_sma_positions
    ...
    sig = walk_forward_meta_sma_positions(
        close,
    ...
    print()
    print("=== Meta-labeling: SMA primary + logistic gate (when primary long) ===")
    print(
        f"Ticker: {args.ticker}  |  SMA {args.fast}/{args.slow}  |  "
        f"train_window={args.train_window}  n_lags={args.n_lags}  P(up)>{args.prob_threshold:g}"
    )
    print(f"Commission (per side, on notional): {COMMISSION_RATE:.4%}")
    print()
    print(f"{'Metric':<22} {'Meta SMA':>16} {'Buy & hold':>16}")
    print("-" * 56)
    ...
    title = f"{args.ticker} — meta SMA (lags={args.n_lags}, W={args.train_window}) vs buy & hold"
    eq_path = out_dir / "equity_drawdown.png"
    px_path = out_dir / "price_meta_prob.png"
    ...
    plot_price_meta_prob(
        plot_df,
        prob_threshold=args.prob_threshold,
        title=f"{args.ticker} — SMA + meta P(up) (green = long, executed)",
        save_path=px_path,
        show=show_plots,
    )
