"""Charts for meta-labeling SMA backtest vs buy-and-hold."""
...
def plot_price_meta_prob(
    ...
    """
    df: close, sma_fast, sma_slow, primary_long, prob_up, position (executed).
    """
    fig, axes = plt.subplots(2, 1, figsize=(11, 7), sharex=True, gridspec_kw={"height_ratios": [2, 1]})
    ...
    if "sma_fast" in df.columns and "sma_slow" in df.columns:
        ax_p.plot(df.index, df["sma_fast"], label="Fast SMA", color="#2ca02c", linewidth=1.0)
        ax_p.plot(df.index, df["sma_slow"], label="Slow SMA", color="#d62728", linewidth=1.0)
    ...
    ax_r = axes[1]
    ax_r.plot(df.index, df["prob_up"], label=r"$P(\mathrm{up})$ (meta)", color="#9467bd", linewidth=1.0)
