...
from bbc_analysis import fetch_bbc_news, analyze_sentiment_full_article


def collect_bbc_news_sentiment(
    queries,
    num_articles_per_query=3
):
    """
    Fetch BBC news, run full-article sentiment, return rows for CSV.
    """
    rows = []
...
def main():
    ...
    rows = collect_bbc_news_sentiment(
        queries=queries,
        num_articles_per_query=3,
    )
    ...
    output_file = f"bbc_sentiment_{timestamp}.csv"
