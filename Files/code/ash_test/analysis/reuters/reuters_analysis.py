...
def fetch_article_content(url: str) -> str:
    ...
    try:
        ...
        # Reuters articles are typically in div with class="ArticleBodyWrapper"
        content_div = soup.find("div", class_="ArticleBodyWrapper")
        if content_div:
...
def fetch_reuters_world_news(query, num_articles=5):
    """
    Additional fetch from Reuters World News for broader coverage.
    """
    rss_url = "https://www.reuters.com/rssFeed/worldNews"
    
    feed = feedparser.parse(rss_url)
    
    news_items = feed.entries[:num_articles * 2]
...
def summarize_sentiments(articles):
    ...
    total = len(articles)
    print("\n--- Reuters Sentiment Summary (Full Article) ---")
...
def main():
    ...
    for query in queries:
        print(f"Fetching Reuters articles for '{query}'...\n")
        # Fetch from business news
        business_articles = fetch_reuters_news(query, num_articles_per_query)
